...
export default function MainLogic() {
        ...
        db.transaction((tx) => {
            ...
            tx.executeSql(
              ...
              FOREIGN KEY (duplicate_fid) REFERENCES Files_Record (id)
                ...
                (error) => console.error('Error creating Duplicates_Record table:', error)
            );
            tx.executeSql(
                ...
                (_, error) => console.error("Error creating Initial_Scan_Results table:", error)
              );
            // Create indexes
            tx.executeSql(
            ...
            tx.executeSql(
                ...
                (error) => console.error('Error creating index on similarity_score:', error)
            );
        });
    };
    ...
    const computeAndCompareHash = async (filePath) => {
        try {
            ...
            if (results.length > 0) {
                ...
                await displayNotification('Duplicate files detected', similarFilesWithPath.length)
                for (const result of results) {
                    ...
                    const duplicateFileId = await insertHashIntoDatabase(filePath, hash, filetype, fileName, fileSizeKB);
                ...
                await insertHashIntoDatabase(filePath, hash, filetype, fileName, fileSizeKB);
            }
        } catch (error) {
            ...
            db.transaction((tx) => {
                tx.executeSql(
                    ...
                    (_, results) => {
                        ...
                        for (let i = 0; i < results.rows.length; i++) {
                            const row = results.rows.item(i);
                            knownFiles[row.original_fid] = row.duplicate_fid + ' :@: ' + row.similarity_score; // Populate knownFiles with file paths and hash
                        }
                        console.log(knownFiles)
                        resolve(knownFiles);
                    },
    ...
    const dropTable = () => {
        db.transaction((tx) => {
            tx.executeSql(`DROP TABLE IF EXISTS Duplicates_Record`, [],
                () => console.log(`Table "Duplicates_Record" has been dropped.`),
                (error) => console.error(`Error dropping table "Duplicates_Record":`, error)
            );
        });
    };
    const truncateDatabaseFiles = () => {
        db.transaction((tx) => {
            tx.executeSql('DELETE FROM Files_Record', [],
                () => console.log('All records from "Files_Record" table have been cleared.'),
    ...
    const truncateDatabaseDuplicates = () => {
        db.transaction((tx) => {
            tx.executeSql('DELETE FROM Duplicates_Record', [],
                () => console.log('All records from "Duplicates_Record" table have been cleared.'),
                (error) => console.error('Error clearing "files" table:', error)
            );
        });
    };
            ...
            (_, result) => {
              if (result.rows.length > 0) {
                ...
                tx.executeSql(
                  ...
                  (error) => console.error('Error deleting from Duplicates_Record:', error)
                );
                ...
                tx.executeSql(
                  ...
                  (error) => console.error('Error deleting from Files_Record:', error)
                );
              } else {
    ...
    return (
        <SafeAreaView style={styles.container}>
            <View>
                ...
                <Button title="Truncate Duplicate DB" onPress={truncateDatabaseDuplicates} />
